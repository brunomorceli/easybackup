...
module.exports.list = (dumpPath, db) => {
  ...
  .forEach((filename) => {
    ...
    const dateNumber = getDateNumber(filename);
    const date = getDateText(filename);
    const dbName = filename.split('@')[1].replace('.gz', '');

    if (!databases[dbName]) { databases[dbName] = {}; }

    if (!getMostRecent(dateNumber, databases[dbName].dateNumber)) {
      return;
    }
...
module.exports.restore = (dumpPath, db, date) => {
  ...
  if (date === 'last') {
    files.forEach(function(filename) {
      chosenFile = !chosenFile ? filename : getMostRecent(chosenFile, filename);
    });
  }
  else {
  ...
  if (!chosenFile) { return console.log('[WARNING] No backup file found.'); }
